...
const pathResolve = require("./path").pathResolve;
...
module.exports = {
  ...
  fileGetList:getFiles,
  getFileContent,
};

function getFileContent(filePath,options = {}){
  const {async = false} = options;
  const funcName = async ? 'readFile' : 'readFileSync';
  return fs[funcName](pathResolve(filePath),options);
}
...
async function eachFile(filePath,func,options = {}){
  const result = [];
  async function exec(filePath,func,options = {},isSrc){
    filePath = resolve(filePath);
    if(!isSrc && !isValidPath(filePath,options)){
      return;
    }
  ...
  await exec(filePath,func,options,true);
  return result;
}
